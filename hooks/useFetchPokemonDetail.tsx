...
export default function useFetchPokemonDetail() {
  ...
  const parseEvolutions = async (chain: any) => {
    ...
    while (current) {
      ...
      const data = await pokemonDetailService.loadEvolutions(name);
      const sprite =
  ...
  const fetchPokemon = async (id: number) => {
    try {
      ...
      // 1. O Axios já busca os objetos JS direto em paralelo
      const [pokemonData, speciesData] = await Promise.all([
        pokemonDetailService.loadPokemon(id),
        pokemonDetailService.loadPokemonSpecie(id),
      ]);

      // 2. Busca a evolução com segurança (usando o objeto retornado do Axios)
  ...
  // ─── Retorno unificado de dados, estilos e funções ──────────────────────────
  return {
    // 📊 Dados do Pokémon
    pokemon,
    evolutions,

    // ⚙️ Estados de controle da tela
    loading,
    error,
    background, // O array com as 3 cores do gradiente já atualizado

    // 🎨 Utilitários de Estilização Prontos (Mapeamento de Cores)
    TYPE_COLORS, // Para renderizar as tags de tipo com as opacidades corretas
    STAT_NAMES, // Para traduzir "hp" -> "HP", "attack" -> "Ataque", etc.
    STAT_COLORS, // Para colorir as barras de progresso de cada atributo
    MAX_STAT, // O valor fixo de 255 para calcular a largura da barra (%)

    // 🚀 Ações
    refetchPokemon: () => fetchPokemon(id), // Caso queira um botão de "Tentar Novamente" se der erro
  };
} // <-- Fechamento final do hook useFetchPokemonDetail
