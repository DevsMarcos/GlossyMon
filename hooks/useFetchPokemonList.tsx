...
export default function useFetchPokemonList() {
  ...
  const fetchPokemons = async (currentOffset: number) => {
    ...
    try {
      ...
      const detalhes: Pokemon[] = await Promise.all(
        listData.results.map(async (item: { url: string }) => {
          ...
          return {
            ...
            name: detData.name,
            sprite:
              detData.sprites.other["official-artwork"].front_default ??
      ...
      setPokemons((prev) =>
        currentOffset === 0 ? detalhes : [...prev, ...detalhes],
      );
  ...
  const onSearchChanged = async () => {
    ...
    const matched = allNames.filter((p) => p.name.includes(term)).slice(0, 20);
